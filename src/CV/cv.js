...
export let CV = {
	...
	clientLogin: function(apiToken,username,password,callbackSuccess,callbackFailure ){
		this.call(
			"clientLogin",
			{
				apiToken: apiToken,
	...
	getClientConfig: function(sessionId){
		this.call(
			"getClientConfig",
			{
				// udid : localStorage.getItem("udid"),
			...
			function(result){
				if (result['success']){
					var clientConfig = JSON.stringify(result['answer'])
